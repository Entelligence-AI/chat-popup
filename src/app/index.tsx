...
import { fetchRepositoryAgent } from '@/api/fetchRepositoryAgent';
import storeConversation from '@/api/storeConversation';
import createConversationForRepository from '@/api/createConversationForRepository';
...
const MyCustomAdapter = ({ apiKey, repoName, organization }: AnalyticsData) => {
  return {
    async *run({
      ...
      const response = await fetchRepositoryAgent({
        question,
        apiKey,
        organization,
        repoName,
      ...
      const conversationUUID = await createConversationForRepository(
        {
